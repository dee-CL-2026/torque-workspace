...
- **Project tags**: Tasks now have a Project column (torque, candid, tslc, infra, personal, general). Dashboard should support filtering by project.
- **Project pages**: Future — clicking a project tag opens a project-specific view showing only tasks/backlog for that project.
...
### f. Activity Feed
- Last 10 agent actions with timestamps
- Shows what actually happened today
- Source: data/activity-log.json (generated by agents after completing tasks)

### g. Session Health
- Current context % for main session
- Active session count
- Source: data/dash-metrics.json (extended)
...
- **dash-metrics.json is the primary data source** — generated by secretary/maintenance cron, consumed by dashboard. No live API calls from static site.
- **Cost data**: Collector agent logs costs to data/cost-log.json after each spawn. Dashboard reads this for cost tracker.
- **Cron timeline**: Snapshot approach — maintenance cron writes cron status to dash-metrics.json rather than dashboard calling gateway API directly.
